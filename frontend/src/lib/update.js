// Optional Trax+ update check — configured only when a Trax+ release endpoint is supplied.
//
...
// The test-mode fallback keeps the parser tests hermetic without making a real Trax+ build
// query the upstream openGym release service. Production builds require an explicit endpoint.
const RELEASES_URL = import.meta.env.VITE_RELEASES_URL
  || (import.meta.env.MODE === 'test' ? 'https://gitlab.com/api/v4/projects/DuarteSantos8%2Fopengym/releases' : '')
export const updateChecksConfigured = () => Boolean(RELEASES_URL)
...
export async function checkForUpdate() {
  if (!RELEASES_URL) return { hasUpdate: false, latestVersion: __APP_VERSION__, apkUrl: null, hashUrl: null }
...
export async function downloadAndInstall(url, expectedHash = null, onProgress = null) {
  if (!MOBILE) {
    ...
    if (RELEASES_URL) window.open(RELEASES_URL, '_blank', 'noopener')
    return
  }
  ...
  const fileName = 'trax-plus-update.apk'
