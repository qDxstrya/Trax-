...
import { cleanWorkoutStats } from './trax-plus.js'

export function buildCompletedWorkout(active, { end = Date.now(), prs = [], snapshotFor, workoutStats } = {}) {
  ...
  return {
    ...
    ...(Object.keys(cleanWorkoutStats(workoutStats)).length ? { workoutStats: cleanWorkoutStats(workoutStats) } : {}),
  }
}
