...
import { STANDALONE } from '../lib/demo.js'
...
export default function Settings() {
  ...
  const { update, replaceState, setUser, pullState, pushState, signOut, signOutAll, disconnectServer } = useStore()
  ...
  const onUpdateRowClick = () => {
    ...
    if (updateInfo.apkUrl) {
      ...
      toast(t('No Trax+ release is configured for this build.'))
    }
  }
  ...
  return <div className="narrow">
    ...
    {/* ---------- account (standalone and mobile local builds have nothing to sign in to) ---------- */}
    <Section title={MOBILE ? (user ? t('Your server') : t('Your data')) : STANDALONE ? t('Your data') : t('Account')}>
      {MOBILE ? (user ? <>
        ...
        <Row icon="link" iconTint="var(--indigo)" title={t('Connect to my server')} subtitle={t('Sync this device to your own self-hosted Trax+ instead.')} accessory="chevron"
          onClick={() => useUI.getState().openSheet(close => <ConnectSheet close={close} />)} />
      </>) : STANDALONE ? <>
        <Row icon="lock" iconTint="var(--acc)" title={t('Trax+ local')} subtitle={t('Your workouts and settings stay on this device. No demo data, no account required.')} />
      </> : user ? <>
        <Row icon="personCircle" iconTint="var(--grey)" title={user.name} subtitle={t('Signed in with passkey — data syncs to this profile.')} />
        {user.admin && <Row icon="wrench" iconTint="var(--indigo)" title={t('Admin dashboard')} accessory="chevron" onClick={() => nav('/admin')} />}
        <Row icon="link" iconTint="var(--blue)" title={t('Pair the mobile app')} subtitle={t('Connect the Trax+ app on your phone to this account.')} accessory="chevron"
    ...
    {!user && !STANDALONE && !MOBILE && <p className="sect-f" style={{ marginTop: -18, marginBottom: 22 }}>{t('Guest mode — data lives only in this browser.')}</p>}
    ...
    {MOBILE && <Section title={t('AI Coach')}>
      <Row icon="sparkles" iconTint="var(--acc)" title={t('AI Coach')} accessory="chevron"
        subtitle={coachLocal?.mode === 'server' ? t('Runs on your Trax+ server') : coachLocal?.mode === 'byok' ? t('Runs on this phone with your own API key') : t('Off — choose how the Coach should run.')}
    ...
    {/* ---------- appearance ---------- */}
    <Section title={t('Appearance')} footer={STANDALONE || MOBILE ? undefined : t('synced with your profile')}>
    ...
    {!MOBILE && <Section title={t('Tip')}>
      <Row icon="lightbulb" iconTint="var(--yellow)"
        ...
        subtitle={t('to install Trax+ as a full-screen app.') + ' ' + (user ? t('Your data syncs with your profile — sign in anywhere to see it.') : t('Guest data stays on this device — export a backup now and then!'))} />
    </Section>}
    ...
    {(!MOBILE || android) && <Section title={t('Updates')}
      ...
      {MOBILE
        ? <Row icon="download" iconTint="var(--acc)"
            title={updateInfo?.hasUpdate ? t('Update to Trax+ v{0}', updateInfo.latestVersion) : t('Check for updates')}
        ...
        : <Row icon="download" iconTint="var(--acc)" title={t('Get the Android app')}
            subtitle={t('Configure VITE_RELEASES_URL in your build to publish downloads.')} accessory="chevron"
            onClick={() => toast(t('No Trax+ release is configured for this build.'))} />}
    </Section>}
    ...
    <div className="dim small" style={{ textAlign: 'center', marginTop: 4, lineHeight: 1.6 }}>
      Trax+ v{__APP_VERSION__} · {t('free & open source (AGPL v3)')}<br />
      <a href="https://github.com/qDxstrya/opengym" target="_blank" rel="noopener">source code</a> · exercise data: hasaneyldrm/exercises-dataset (MIT)<br />
  ...
  return <>
    <Section
      ...
      footer={on && S.reminder?.on
        ...
        : null}
    >
      <Row icon="bell" iconTint="var(--red)" title={t('Push notifications')} subtitle={t('Rest-timer alerts, even if Trax+ is closed.')}>
...
function PairSheet({ close }) {
  ...
  return <>
    ...
    <div className="muted small" style={{ marginBottom: 14 }}>
      {t('On the Trax+ app, choose “Connect to my server”, then enter this address and the code below. It expires in 5 minutes.')}
    </div>
