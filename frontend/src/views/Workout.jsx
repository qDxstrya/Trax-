...
import { cleanCardioMetrics, repeatableSet } from '../lib/trax-plus.js'
...
function ExerciseBlock({ entryIdx, compact, dense, onToggle, onToggleSide, onField, onRepeatSet, onAddSet, onRemoveSet, onAddWarmup, onRemoveSetAt, onStartTimed, onPairPrev, onPairNext, onSetRowRef, onProgressionSettings, onSwap, onMoveUp, onMoveDown, canMoveUp, canMoveDown, onRemoveExercise, busy }) {
  const S = useStore(s => s.S)
  ...
  return <>
    ...
    <div className="card" style={{ marginTop: 10, marginBottom: 0 }}>
      ...
      {entry.sets.map((s, i) => {
        ...
        const repeat = i > 0 && repeatableSet(entry.sets, i)
        ...
        return <div key={i}>
          ...
          {cardio && <div className="row" style={{ gap: 6, flexWrap: 'wrap', margin: '5px 0 0 36px' }}>
            <label className="small dim" style={{ flex: '1 1 90px' }}>{t('Distance (km)')}<NumberField decimal nullable value={s.distance ?? ''} onChange={v => onField(i, 'distance', v)} /></label>
            <label className="small dim" style={{ flex: '1 1 90px' }}>{t('Reps')}<NumberField nullable value={s.cardioReps ?? ''} onChange={v => onField(i, 'cardioReps', v)} /></label>
            <label className="small dim" style={{ flex: '1 1 90px' }}>{t('Pace (min:sec)')}<input className="input" inputMode="decimal" value={s.pace ?? ''} placeholder="5:30" onChange={e => onField(i, 'pace', e.target.value)} /></label>
          </div>}
          {repeat && !s.done && !warm && <button type="button" className="chip add" style={{ marginTop: 5 }} onClick={() => onRepeatSet(i)}>
            <Icon name="reset" />{t('Repeat last set')}
          </button>}
...
function ActiveWorkout() {
  ...
  const setField = (idx, i, field, v) => mutEntry(idx, e => {
    if (['distance', 'cardioReps', 'pace'].includes(field)) {
      const cleaned = cleanCardioMetrics({ [field]: v })
      if (cleaned[field] == null) delete e.sets[i][field]
      else e.sets[i][field] = cleaned[field]
    } else if (v == null) delete e.sets[i][field]; else e.sets[i][field] = v
  ...
  const addSet = idx => mutEntry(idx, e => {
    ...
    if (m === 'cardio') e.sets.push({ ...cleanCardioMetrics({ ...(l || {}), min: l ? l.min : (e.target.min || 20), speed: l ? l.speed : (e.target.speed || 8) }), done: false })
  ...
  const blockProps = idx => ({
    ...
    onRepeatSet: i => update(s => {
      const entry = s.active?.entries?.[idx]
      const source = entry ? repeatableSet(entry.sets, i) : null
      if (!entry || !source || entry.sets[i]?.done) return
      const copy = { ...source, done: false }
      delete copy.phase
      delete copy.warmup
      entry.sets[i] = copy
    }, true),
