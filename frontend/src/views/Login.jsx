...
export default function Login() {
  ...
  // Standalone web build: local-only, no backend or account required.
  if (STANDALONE) return (
    <div className="narrow" style={wrap}>
      {head}
      <div className="muted" style={{ marginBottom: 30 }}>{t('Your training stays on this device.')}</div>
      <Button variant="primary" icon="sparkles" onClick={() => setGuest(true)}>{t('Open Trax+')}</Button>
    </div>
  )
  ...
  return (
    <div className="narrow" style={wrap}>
      ...
      {webauthnOK() ? <>
        ...
        ? t("This browser doesn't support passkeys — you can still use Trax+ locally on this device.")
